...
router.get('/plot-summary', auth, (req, res) => {
  try {
    const db = getDB();
    const rows = db.prepare(`SELECT plot, SUM(quantity_kg - wastage_kg) AS total_kg, SUM(total_revenue) AS revenue, COUNT(*) AS batches FROM harvest_records GROUP BY plot`).all();
